...
* **Datasets Compatibility:** Adapted to process heavy laboratory imaging datasets (totaling 132 GB of images, with single sensor runs ranging between 30-50 GB) locally, avoiding cloud storage limitations.
* **Task-Specific Application:** Tailored for only automated segmentation, feature extraction and processing of NGN2-driven neural differentiation datasets.
...
2. `bandpass_segmentation.py` — Image preprocessing and bandpass filtering.
3. `cropping_single_cells.py` — Region of interest (ROI) extraction and single-cell isolation.
4. `features_extraction.py` — Morphological and signal feature extraction from segmented cells.
5. `classification.py` — Downstream data classification and clustering analysis.
...
```bash
pip install numpy pandas matplotlib scikit-learn scikit-image opencv-python Pillow mahotas tqdm
```
...
*“Visualizing H3K9ac, H3K9me3, and H3K36me2/3 dynamics in living cells during NGN2-induced neuronal differentiation”*.
